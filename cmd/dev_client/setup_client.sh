...
# Create pipe to redirect input to dev_client CLI
pipe="dev_client_in_${id}"
mkfifo $pipe

# Create temporary file to store dev_client CLI output
out="dev_client_out_${id}.txt"

# Redirect pipe to dev_client binary, and also store its output in a temporary file
(sudo ./dev_client < $pipe 2>&1 | tee $out &) # Use sed to copy the combined output stream to the specified log file, until the test suite's exit code is found &)
...
function clean_exit () {
    ...
    # Kill process continuosly feeding input to dev_client
    ...
    # Remove temporary dev_client output file
    sudo rm $out

    # Terminate dev_client, which remains open with userspace WireGuard
    dev_client_pid=$(pgrep dev_client) && sudo kill $dev_client_pid
